...
export function AddDirectiveDialog({ open, onClose, onAdd }: AddDirectiveDialogProps) {
  ...
  // ряд: у набора частей журнала и списка типов ответа поле одно, а у
  // умолчаний фазы их четыре, а у правки целей рядом с полями стоит ещё и
  // секция с параметрами — в узком окне она переносится на строку за строкой.
  const wide = form !== null && (form.arg === 'actions' || form.arg === 'exclusion');
